...
public:
        ...
        unsigned short regBase = 0) 
    {
        ...
        if (functionCode == 0x03)
        {
            ...
            // Check requested range falls within [regBase, regBase+2)
            if (startAddr >= regBase && (startAddr + quantity) <= (regBase + 2))
            {
                ...
                for (int i = 0; i < (int)quantity; i++)
                {
                    ...
                    if (offset == 0) regValue = (unsigned short)m_temp;
                    else if (offset == 1) regValue = (unsigned short)m_rpm;
